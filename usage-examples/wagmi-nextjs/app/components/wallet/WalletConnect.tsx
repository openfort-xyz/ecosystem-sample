      ...
      <div className="flex flex-col items-center space-y-2 items-center">
        <div className="flex flex-col gap-4 items-center">
          <div className="flex gap-2 items-center">
            ...
            <Button
              variant={selectedWallet === 'wagmi' ? 'default' : 'outline'}
              className="flex-1 font-medium justify-start rounded-full"
              onClick={() => setSelectedWallet('wagmi')}
            >
              <div className={selectedWallet === 'wagmi' ? 'text-default' : 'text-outline'}>
                <WagmiLogo />
              </div>
              <span className={selectedWallet === 'wagmi' ? 'text-default' : 'text-outline'}>
                Wagmi
              </span>
            </Button>
            
            <Button 
              variant={selectedWallet === 'connectkit' ? 'default' : 'outline'}
              className="flex-1 justify-start rounded-full"
              onClick={() => setSelectedWallet('connectkit')}
            >
              <div className={selectedWallet === 'connectkit' ? 'text-default' : 'text-outline'}>
                <ConnectKitLogo />
              </div>
              <span className={selectedWallet === 'connectkit' ? 'text-default' : 'text-outline'}>
                ConnectKit
              </span>
            </Button>

            <Button 
              variant={selectedWallet === 'rainbowkit' ? 'default' : 'outline'}
              className="flex-1 justify-start rounded-full hidden md:flex"
              onClick={() => setSelectedWallet('rainbowkit')}
            >
              <div className={selectedWallet === 'rainbowkit' ? 'text-default' : 'text-outline'}>
                <RainbowLogo />
              </div>
              <span className={selectedWallet === 'rainbowkit' ? 'text-default' : 'text-outline'}>
                RainbowKit
              </span>
            </Button>
          </div>
          <p className="text-gray-500 font-light text-xs md:text-xs">
          ...
          {selectedWallet === 'wagmi' ? (
            <div className='flex items-center'>
              <Button
                ...
                className="inline-flex font-bold items-center gap-2 rounded-lg shadow-sm px-6 py-5"
              >
                Sign in
              </Button>
            </div>
      ...
      {address && (
        <div className="flex gap-3 bg-white justify-center items-center">
          <Button
            ...
            className="font-mono flex text-sm break-all px-3 py-2 rounded-md justify-between items-center bg-gray-100"
          >
            <a
            href={chain?.blockExplorers?.default.url + '/address/' + address}
            target="_blank"
            rel="noopener noreferrer"
            className="bg truncate text-black hover:text-black/80 text-sm"
            >
              {truncateAddress(address)}
            </a>
            {copied ? (
              <Check className="w-4 h-4 text-gray-500 flex-shrink-0 ml-2" />
            ) : (
              <Copy className="w-4 h-4 text-gray-500 flex-shrink-0 ml-2" />
            )}
          </Button>
          <Button
            onClick={handleDisconnectWallet}
            className="flex bg-gray-400 text-black items-center justify-center gap-2 rounded-md"
            size="sm"
          >
